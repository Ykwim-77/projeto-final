  ...
  login(email: string, password: string): Observable<any> {
    ...
    return this.http.post<any>(
      ...
      tap(response => {
        ...
        // ✅ VERIFICAÇÃO DIRETA: Se tem mensagem de erro, lança exceção
        if (response.mensagem && response.mensagem === 'Email ou senha incorretos') {
          console.log('❌ Credenciais inválidas detectadas');
          throw new Error('CREDENTIALS_ERROR');
        }
        
        // ✅ Se tem usuário, é sucesso
        if (response.usuario) {
      ...
      catchError((error: any) => {
        console.log('🔧 Processando erro no catchError:', error);
        
        // Se é o erro que lançamos manualmente
